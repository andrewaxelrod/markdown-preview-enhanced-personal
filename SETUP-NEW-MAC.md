...
Before loading the login agent the script imports the app once in the foreground, so a broken
environment shows its traceback right there. Then it waits up to five minutes for the server —
a first start on a managed Mac can be slow while endpoint security scans the new files — and,
if it never answers, prints the agent's state and the last lines of its log.
...
| Step 6 prints `curl: (7) Failed to connect` or waits and times out    | The first start can take minutes on a managed Mac. `curl -s http://127.0.0.1:8880/health` again; then `tail -40 ~/Library/Logs/kokoro-fastapi.log` and `launchctl print gui/$(id -u)/com.andrew.kokoro-fastapi \| grep -E "state\|pid\|last exit"`. To watch it start in the foreground: `launchctl bootout gui/$(id -u)/com.andrew.kokoro-fastapi; ~/.local/share/kokoro-fastapi/start-local.sh`. |
